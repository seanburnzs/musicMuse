...
class MusicMuse:
    ...
    def parse_natural_language(self, query_text):
        ...
        parsed = {
            "original_query": query_text,  # Store the original query text
            "year": None,
            "day_of_week": None,
            "time_after": None,
            "time_before": None,
    ...
    def build_sql_query(self, parsed):
        ...
        # Add a final check to make sure the SQL is valid
        if not sql or not params:
            # Return a simple query that indicates something went wrong
            return "SELECT %s AS error_msg;", ["Failed to build a valid SQL query for your request."]
    ...
    def format_response(self, parsed, results):
        ...
        if isinstance(results, list) and len(results) > 0 and isinstance(results[0], tuple) and results[0][0] == "Error executing query":
            return f"Sorry, I couldn't process that query: {results[0][1]}"
        
        if isinstance(results, dict) and "error" in results:
            return f"Sorry, I couldn't process that query: {results['error']}"
        
        entity_type = parsed.get("entity_type", "track")
        action = parsed.get("action", "top")
        use_count = parsed.get("use_count", False)
        
        # Get original query for display
        original_query = parsed.get("original_query", "")
        
        # Start response with the query
        response = f'<p style="color: #888; font-weight: 300;">{original_query}</p>\n\n'
        
        # Special handling for different action types
        if action == "first":
            # Handle "first listen" type queries
            if not results or len(results) == 0:
                return f"I couldn't find any record of when you first listened to that."
            
            entity = results[0][0] if len(results) > 0 and len(results[0]) > 0 else "Unknown"
            timestamp = results[0][1] if len(results) > 0 and len(results[0]) > 1 else "Unknown date"
            ...
            if entity_type == "artist":
                return response + f"You first listened to {entity} on {timestamp}."
            elif entity_type == "track":
                artist = results[0][2] if len(results) > 0 and len(results[0]) > 2 else "Unknown artist"
                return response + f'You first listened to <span class="track-name">"{entity}"</span> by {artist} on {timestamp}.'
            elif entity_type == "album":
                artist = results[0][2] if len(results) > 0 and len(results[0]) > 2 else "Unknown artist"
                return response + f'You first listened to the album <span class="album-name">"{entity}"</span> by {artist} on {timestamp}.'
        
        elif action == "skipped":
            # Handle skipped tracks queries
            if not results or len(results) == 0:
                return response + f"I couldn't find any skipped {entity_type}s matching your query."
            
            for i, result in enumerate(results, 1):
                if entity_type == "track":
                    entity = result[0] if len(result) > 0 else "Unknown track"
                    sub_entity = result[1] if len(result) > 1 else "Unknown artist"
                    response += f'<span class="track-name">{entity}</span> by {sub_entity}<br>'
                elif entity_type == "artist":
                    entity = result[0] if len(result) > 0 else "Unknown artist"
                    response += f'<span class="artist-name">{entity}</span><br>'
                elif entity_type == "album":
                    entity = result[0] if len(result) > 0 else "Unknown album"
                    sub_entity = result[1] if len(result) > 1 else "Unknown artist"
                    response += f'<span class="album-name">{entity}</span> by {sub_entity}<br>'
            
            return response
        
        # Default handling for "top" action and other queries
        if not results or len(results) == 0:
            return response + f"I couldn't find any {entity_type}s matching your query."
    ...
    def _format_time_constraints(self, parsed):
        """Format time constraints from the parsed query into a readable string."""
        time_range = parsed.get("time_range", {})
        time_type = time_range.get("type", "all_time")
        ...
        if time_type == "all_time":
            ...
            return f"in {year}"
        elif time_type == "custom" and "start_date" in time_range:
            start_date = time_range.get("start_date")
            end_date = time_range.get("end_date", "now")
    ...
    def _format_ms_to_time(self, ms):
        ...
        if days > 0:
            return f"{days} day{'s' if days != 1 else ''} {hours % 24} hour{'s' if hours % 24 != 1 else ''}"
        elif hours > 0:
            return f"{hours} hour{'s' if hours != 1 else ''} {minutes % 60} minute{'s' if minutes % 60 != 1 else ''}"
        elif minutes > 0:
